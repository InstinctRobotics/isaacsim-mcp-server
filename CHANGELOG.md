...
### Changed
- **`get_joint_config` reports an angular drive's `stiffness` and `damping` per radian, not per degree.** `UsdPhysicsDriveAPI` stores an angular drive's gains per degree and the USD-to-PhysX parse multiplies them by 180/π, so the value this tool returned was not the value the simulation ran. On a Franka Panda at `metersPerUnit = 1.0`, an authored `400 / 80` on `panda_joint1` is `22918.31 / 4583.66` at runtime, while `panda_finger_joint1` — prismatic, gains per stage unit — passes through unchanged at `400.00 / 80.00`. The same payload already converts limits and positions to radians and metres, so an agent that sized a gain from those numbers and wrote it back was off by 57.3× on every revolute joint, with nothing in the response saying which unit it had been handed. The conversion lives in `adapters/units.py` beside the joint-limit split, and is keyed on the drive instance name rather than the joint type: `get_joint_config` finds a drive by probing for it, so an asset whose prismatic joint carries an angular drive is already reported as angular — and that drive does store per-degree gains. Each joint now carries `gain_units`.
