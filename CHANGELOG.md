...
### Changed
- **`get_joint_config` reports an angular drive's `stiffness` and `damping` per radian, not per degree.** USD stores angular gains per degree and the engine consumes them per radian, so the number returned was 57.3× off from the one the simulation ran — in the same payload whose limits and positions were already radians. Each joint now carries `gain_units`, and the docstring says to divide by 180/π when writing a gain back through `execute_script`.
